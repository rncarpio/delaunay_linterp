...
protected:
 ...
 /*{\Xop removes the mark bits from all marked simplices reachable from $s$.}*/
  ...
  void dimension_jump(Simplex_handle S, Vertex_handle x, std::vector<Simplex_handle> *p_modified_simplices=NULL);
...
public:
  ...
  Vertex_handle insert(const Point_d& x, std::vector<Simplex_handle> *p_modified_simplices=NULL);
...
Convex_hull_d<R>::insert(const Point_d& x, std::vector<Simplex_handle> *p_modified_simplices=NULL)
{ 
  ...
  if (current_dimension() == -1) { 
    ...
    start_facet_ = origin_simplex_;
    z = new_vertex(x);                num_of_vertices ++; 
    associate_vertex_with_simplex(origin_simplex_,0,z);			if (p_modified_simplices != NULL) { p_modified_simplices->push_back(origin_simplex_); }
      // z is the only point and the peak 
    associate_vertex_with_simplex(outer_simplex,0,anti_origin_); 	if (p_modified_simplices != NULL) { p_modified_simplices->push_back(outer_simplex); }
    set_neighbor(origin_simplex_,0,outer_simplex,0); 
    typename R::Point_to_vector_d to_vector =
      kernel().point_to_vector_d_object();
    quasi_center_ = to_vector(x);
    ...
    typename R::Point_to_vector_d to_vector =
      kernel().point_to_vector_d_object();
    quasi_center_ = quasi_center_ + to_vector(x);
    dimension_jump(origin_simplex_, z, p_modified_simplices); 
    clear_visited_marks(origin_simplex_); 
    Simplex_iterator S;
    forall_rc_simplices(S,*this) compute_equation_of_base_facet(S);
    num_of_unbounded_simplices += num_of_bounded_simplices;
      ...
      case 1:
        { num_of_vertices++; 
          ...
          for (it = visible_simplices.begin(); 
            ...
            Simplex_handle S = *it; 
            associate_vertex_with_simplex(S,0,z); 		if (p_modified_simplices != NULL) { p_modified_simplices->push_back(S); }              // this simplex is no longer unbounded
            for (int k = 1; k <= dcur; k++) {
              if (!is_base_facet_visible(opposite_simplex(S,k),x)) {
                Simplex_handle T = new_simplex(); 		if (p_modified_simplices != NULL) { p_modified_simplices->push_back(T); }
                NewSimplices.push_back(T);
...
template <class R>
void Convex_hull_d<R>::
dimension_jump(Simplex_handle S, Vertex_handle x, std::vector<Simplex_handle> *p_modified_simplices=NULL)
{ 
  Simplex_handle S_new; 
  S->visited() = true; 
  associate_vertex_with_simplex(S,dcur,x); 			if (p_modified_simplices != NULL) { p_modified_simplices->push_back(S); }
  if ( !is_unbounded_simplex(S) ) { // S is bounded
    S_new = new_simplex(); 
    set_neighbor(S,dcur,S_new,0); 
    associate_vertex_with_simplex(S_new,0,anti_origin_); 	if (p_modified_simplices != NULL) { p_modified_simplices->push_back(S_new); }
    for (int k = 1; k <= dcur; k++) {
  ...
  for (int k = 0; k <= dcur - 1; k++) {
    if ( !opposite_simplex(S,k) -> visited() ) {
      dimension_jump(opposite_simplex(S,k), x, p_modified_simplices); 
    }
  }
